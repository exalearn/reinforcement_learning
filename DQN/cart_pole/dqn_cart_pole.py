...
# DESCRIPTION:
# This code implements Deep Q-Learning (DQN) for the cart pole problem
...
class DQN(object):
    ...
    def select_action(self, state):
        ...
        if rand_num > EPS: # greedy
            ...
            return self.policy_net.forward(state).max(1)[1].item()
        else: # random
    ...
    def plotReward(self):
        f = plt.figure()
        plt.plot(self.listScore)
        plt.title('Reward vs Episode')
        plt.xlabel('Episode')
        plt.ylabel('Duration (Reward)')
        f.savefig("epi_reward.pdf", bbox_inches='tight')
